...
router.post('/register', async ctx => {
  let {user_name = '', user_id = '', user_pwd = ''} = ctx.request.body;
  ...
  try {
    ...
    // 判断user_id是否重复
    let res = await userModel.find_by_user_id(user_id);
    if(res.length != 0){
      ctx.body = {
        code: 409,
        msg: '登录账号重复了,请换一个吧!'
    ...
    let admin = new User({
      user_name,user_id,
      user_pwd: sha1(sha1(user_pwd + SHA1_ADD_STR))
    })
...
router.post('/login', async ctx => {
  let {user_id = '' , user_pwd = '',token = '', code = ''} = ctx.request.body;

  try {
    if(user_id == '' || user_pwd == '' || token == '' || code == ''){
      ctx.body = {
        code: 401,
    ...
    let new_token = create_token(user_id);
...
router.post('/master', async ctx => {
  ...
  try {
    ...
    if(res[0].user_pwd == sha1(sha1(ori_pwd+SHA1_ADD_STR))){
      res = await userModel.update(res[0]._id,{user_pwd:sha1(sha1(re_pwd + SHA1_ADD_STR))});
      console.log(res);
      if(res){
        ctx.body = {
          ctx,code: 200,
